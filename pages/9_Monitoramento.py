...
st.markdown(
    section_header("Sincronização com GitHub Gist", "permite gerenciar destinatários pelo Cloud e o runner local recebe as mudanças"),
    unsafe_allow_html=True,
)

gist_configured = bool(cfg.get("github_gist_id") and cfg.get("github_token"))
if gist_configured:
    st.markdown(
        '<div style="display:inline-flex;align-items:center;gap:0.45rem;'
        'background:rgba(74,222,128,0.09);border:1px solid rgba(74,222,128,0.22);'
        'border-radius:20px;padding:0.22rem 0.9rem;font-size:0.78rem;color:#86EFAC;font-weight:500;margin-bottom:1rem;">'
        '<span style="width:6px;height:6px;border-radius:50%;background:#4ADE80;display:inline-block;"></span>'
        'Gist configurado — runner sincroniza automaticamente</div>',
        unsafe_allow_html=True,
    )

with st.expander("Configurar Gist" if gist_configured else "⚠️ Gist não configurado — clique para configurar"):
    st.markdown(
        '<div style="color:#93C5FD;font-size:0.82rem;margin-bottom:1rem;line-height:1.6;">'
        '1. Acesse <strong>gist.github.com</strong> e crie um novo Gist com o arquivo '
        '<code>config_alertas.json</code> (pode deixar o conteúdo vazio).<br>'
        '2. Copie o <strong>Gist ID</strong> da URL: '
        '<code>gist.github.com/seu-usuario/<strong>GIST_ID</strong></code><br>'
        '3. Crie um <strong>token GitHub</strong> em Settings → Developer settings → '
        'Personal access tokens → com escopo <code>gist</code>.'
        '</div>',
        unsafe_allow_html=True,
    )

    col_gid, col_tok = st.columns(2)
    new_gist_id = col_gid.text_input(
        "Gist ID",
        value=cfg.get("github_gist_id", ""),
        placeholder="abc123def456...",
        key="gist_id_input",
    )
    new_token = col_tok.text_input(
        "GitHub Token",
        value=cfg.get("github_token", ""),
        placeholder="ghp_...",
        type="password",
        key="gh_token_input",
        help="Token com escopo 'gist'. Fica salvo apenas no arquivo local — não é publicado no Gist.",
    )

    col_save_gist, col_test_gist, _ = st.columns([1, 1, 4])
    if col_save_gist.button("Salvar", key="save_gist", type="primary", use_container_width=True):
        cfg["github_gist_id"] = new_gist_id.strip()
        cfg["github_token"]   = new_token.strip()
        _save_config(cfg)
        st.success("Credenciais salvas. O runner irá sincronizar na próxima execução.")
        st.rerun()

    if col_test_gist.button("Testar conexão", key="test_gist", use_container_width=True):
        gid = new_gist_id.strip()
        tok = new_token.strip()
        if not gid or not tok:
            st.error("Preencha Gist ID e Token antes de testar.")
        else:
            from utils.config_loader import _fetch_gist, _push_gist
            with st.spinner("Testando..."):
                result = _fetch_gist(gid, tok)
            if result is not None:
                st.success("Conexão OK — Gist acessível.")
            else:
                ok = _push_gist(cfg, gid, tok)
                if ok:
                    st.success("Conexão OK — Gist criado/atualizado com a config atual.")
                else:
                    st.error("Falha. Verifique o Gist ID e se o token tem escopo 'gist'.")

    if gist_configured:
        st.markdown(
            '<div style="color:#1E3A5A;font-size:0.76rem;margin-top:0.6rem;">'
            'Para remover a sincronização, apague os campos acima e clique Salvar.</div>',
            unsafe_allow_html=True,
        )
